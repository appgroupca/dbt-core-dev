...
,ISNULL([processed_at] AT TIME ZONE 'W. Europe Standard Time','') AS PROCESSED_AT_FULL
,ISNULL(CONVERT(VARCHAR,[order].[processed_at] AT TIME ZONE 'W. Europe Standard Time',23),'') AS PROCESSED_AT
,ISNULL([created_at] AT TIME ZONE 'W. Europe Standard Time','') AS CREATED_AT_FULL
,ISNULL(CONVERT(VARCHAR,[order].[created_at] AT TIME ZONE 'W. Europe Standard Time',23),'') AS CREATED_AT
,ISNULL([cancelled_at] AT TIME ZONE 'W. Europe Standard Time','') AS CANCELLED_AT_FULL
,ISNULL(CONVERT(VARCHAR,[cancelled_at] AT TIME ZONE 'W. Europe Standard Time',23),'') AS CANCELLED_AT

,ISNULL([cancel_reason],'') AS CANCEL_REASON

,ISNULL([closed_at] AT TIME ZONE 'W. Europe Standard Time','') AS CLOSED_AT_FULL
,ISNULL(CONVERT(VARCHAR,[closed_at] AT TIME ZONE 'W. Europe Standard Time',23),'') AS CLOSED_AT
...
FROM [SHOPIFY_PROD].[eu_mck].[order] WITH (NOLOCK)

LEFT JOIN [SHOPIFY_PROD].[eu_mck].[order_tag] AS PRE_ORDER_TAG WITH (NOLOCK)
ON PRE_ORDER_TAG.[order_id] = [id] 
AND PRE_ORDER_TAG.[value] = 'pre-order'

LEFT JOIN [SHOPIFY_PROD].[eu_mck].[order_tag] AS NEWSTORE_FAILED_TAG WITH (NOLOCK)
ON NEWSTORE_FAILED_TAG.[order_id] = [id] 
AND NEWSTORE_FAILED_TAG.[value] = 'newstore_failed'

LEFT JOIN [SHOPIFY_PROD].[eu_mck].[order_tag] AS RISKIFIED_DECLINED_TAG WITH (NOLOCK)
ON RISKIFIED_DECLINED_TAG.[order_id] = [id] 
AND RISKIFIED_DECLINED_TAG.[value] = 'Riskified::declined'

LEFT JOIN [SHOPIFY_PROD].[eu_mck].[order_tag] AS SFCC_TAG WITH (NOLOCK)
ON SFCC_TAG.[order_id] = [id] 
AND SFCC_TAG.[value] = 'sfcc'

LEFT JOIN [SHOPIFY_PROD].[eu_mck].[order_discount_code] AS ORDER_DISCOUNT_CODE WITH (NOLOCK)
