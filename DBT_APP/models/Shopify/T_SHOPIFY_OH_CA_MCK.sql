...
FROM [SHOPIFY_PROD].[ca_mck].[order] WITH (NOLOCK)

LEFT JOIN [SHOPIFY_PROD].[ca_mck].[order_tag] AS PRE_ORDER_TAG WITH (NOLOCK)
ON PRE_ORDER_TAG.[order_id] = [id] 
AND PRE_ORDER_TAG.[value] = 'pre-order'

LEFT JOIN [SHOPIFY_PROD].[ca_mck].[order_tag] AS NEWSTORE_FAILED_TAG WITH (NOLOCK)
ON NEWSTORE_FAILED_TAG.[order_id] = [id] 
AND NEWSTORE_FAILED_TAG.[value] = 'newstore_failed'

LEFT JOIN [SHOPIFY_PROD].[ca_mck].[order_tag] AS RISKIFIED_DECLINED_TAG WITH (NOLOCK)
ON RISKIFIED_DECLINED_TAG.[order_id] = [id] 
AND RISKIFIED_DECLINED_TAG.[value] = 'Riskified::declined'

LEFT JOIN [SHOPIFY_PROD].[ca_mck].[order_tag] AS SFCC_TAG WITH (NOLOCK)
ON SFCC_TAG.[order_id] = [id] 
AND SFCC_TAG.[value] = 'sfcc'

LEFT JOIN [SHOPIFY_PROD].[ca_mck].[order_discount_code] AS ORDER_DISCOUNT_CODE WITH (NOLOCK)
